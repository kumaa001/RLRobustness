...
%% Gradient Descent based Random Search Loop
...
for i1 = 1:Nro
    ...
    % GD being applied on the nominal plant to reduce get the best possible
    % controller cost
    [Jn,pi] = gdSteps(A,Bu,Bw,C,Du,Ak,Bk,Ck,Q,R,W,V,Opt);
    [Ak,Bk,Ck] = K( pi );              
    % Expected Cost for each perturbed plant with the new controller
    for i2=1:Npert        
        Bui = Bp(:,i2);
        Dui = Dp(:,i2);
